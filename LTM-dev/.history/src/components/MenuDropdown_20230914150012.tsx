  ...
  return (
    <Box
      ...
      sx={{
        ...
        "&:hover": {
          bgcolor: item.isActive?.() ? "rgba(84, 83, 83, 0.7)" : "#556EE6",
          color: "white",
        },
        width: "100%",
        display: "flex",
        ...
        alignItems: "center",
        cursor: "pointer",
        justifyContent: !item.isShowDrawerTitle ? "center" : "initial",
        p: 0,
      }}
    >
      <div className="flex items-center gap-x-2 px-3 " onClick={onClick}>
        <div
          className={`flex items-center   ${item.isActive?.() ? "menu-icon-active" : ""
            } ${item.isActive?.() ? "text-[#556EE6]" : "text-[#C8C7C7]"} ${drawerOpen ? "m-1" : "m-auto"
            }`}
        >
          {item.icon}
        </div>
